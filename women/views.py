...
def about(request):
    ...
    women = Women.objects.filter(is_published=True).select_related('cat')
...
def show_category(request, post_slug):
    ...
    tags = Tag.objects.filter(tags__is_published=True).distinct()
    women = Women.objects.filter(cat__slug=post_slug, is_published=True).select_related('cat')
    categories = Category.objects.filter(posts__is_published=True).distinct()
    if not women:
        raise Http404()
...
def show_tag(request, tag_slug):
    ...
    women = Women.objects.filter(tag__slug=tag_slug, is_published=True)
    tags = Tag.objects.filter(tags__is_published=True).distinct()
